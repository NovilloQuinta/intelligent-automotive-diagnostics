...
async function tryPid(
  label: string,
  source: KnowledgeSource,
  pid: PidFormulaSource,
  range: PidRange,
  repo: ObdRepository | undefined,
): Promise<void> {
...
async function exercisePidScenarios(repo: ObdRepository): Promise<void> {
  ...
  await tryPid('origen web (0.3 → 0.7)', KnowledgeSource.Web, RPM_PID, PLAUSIBLE_RPM_RANGE, repo)
  await tryPid(
    'origen mecanico (0.8 → 0.9)',
  ...
  await tryPid('rango 0..10', KnowledgeSource.Web, RPM_PID, IMPOSSIBLE_RANGE, repo)

  console.log('\nPID propietario que el simulador no modela (22 F40C):')
  await tryPid(
    'lectura cruda no soportada',
    KnowledgeSource.Web,
    PROPRIETARY_PID,
    PLAUSIBLE_RPM_RANGE,
    repo,
  ...
  await tryPid('obdRepo undefined', KnowledgeSource.Web, RPM_PID, {}, undefined)
}
