import type { PidDefinition } from '@/domain/entities/pidDefinition.js'
import { Formula } from '@/domain/value-objects/formula.js'
import { PidCode } from '@/domain/value-objects/pidCode.js'

/** PIDs estándar Mode 01 (SAE J1979) — globales, para cualquier vehículo. */
export const STANDARD_MODE_01_PIDS: PidDefinition[] = [
  {
    id: 0,
    pidCode: new PidCode('01', '04'),
    name: 'Calculated Engine Load',
    formula: new Formula('A*100/255'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    description: 'Engine load calculated by the ECU',
  },
  {
    id: 0,
    pidCode: new PidCode('01', '05'),
    name: 'Engine Coolant Temperature',
    formula: new Formula('A-40'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -40,
    maxValue: 215,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '06'),
    name: 'Short Term Fuel Trim — Bank 1',
    formula: new Formula('A*100/128-100'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -100,
    maxValue: 99.2,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '07'),
    name: 'Long Term Fuel Trim — Bank 1',
    formula: new Formula('A*100/128-100'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -100,
    maxValue: 99.2,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '0B'),
    name: 'Intake Manifold Absolute Pressure',
    formula: new Formula('A'),
    unit: 'kPa',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 255,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '0C'),
    name: 'Engine RPM',
    formula: new Formula('(A*256+B)/4'),
    unit: 'rpm',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 16383.75,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '0D'),
    name: 'Vehicle Speed',
    formula: new Formula('A'),
    unit: 'km/h',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 255,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '0E'),
    name: 'Timing Advance',
    formula: new Formula('A/2-64'),
    unit: '° before TDC',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -64,
    maxValue: 63.5,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '0F'),
    name: 'Intake Air Temperature',
    formula: new Formula('A-40'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -40,
    maxValue: 215,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '10'),
    name: 'Mass Air Flow Rate',
    formula: new Formula('(A*256+B)/100'),
    unit: 'g/s',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 655.35,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '11'),
    name: 'Throttle Position',
    formula: new Formula('A*100/255'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 100,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '2F'),
    name: 'Fuel Tank Level Input',
    formula: new Formula('A*100/255'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 100,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '31'),
    name: 'Distance Traveled Since Codes Cleared',
    formula: new Formula('A*256+B'),
    unit: 'km',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '42'),
    name: 'Control Module Voltage',
    formula: new Formula('(A*256+B)/1000'),
    unit: 'V',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65.535,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '46'),
    name: 'Ambient Air Temperature',
    formula: new Formula('A-40'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -40,
    maxValue: 215,
  },
  {
    id: 0,
    pidCode: new PidCode('01', '5C'),
    name: 'Engine Oil Temperature',
    formula: new Formula('A-40'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: -40,
    maxValue: 210,
  },
]

/** Service 09 — Vehicle information PIDs. */
export const STANDARD_MODE_09_PIDS: PidDefinition[] = [
  {
    id: 0,
    pidCode: new PidCode('09', '02'),
    name: 'Vehicle Identification Number (VIN)',
    formula: '' as unknown as Formula,
    unit: '',
    dataBytes: 17,
    pidType: 'ascii',
    confidence: 1.0,
    source: 'manual',
    description: '17-character VIN string',
  },
]

/** PIDs propietarios Mode 22 para Toyota Auris Hybrid. */
export const TOYOTA_AURIS_MODE_22_PIDS: PidDefinition[] = [
  {
    id: 0,
    pidCode: new PidCode('22', '0300'),
    name: 'TCU Odometer',
    formula: new Formula('(A<<24|B<<16|C<<8|D)/10'),
    unit: 'km',
    dataBytes: 4,
    pidType: 'formula',
    confidence: 0.9,
    source: 'manual',
    description: 'Kilometraje almacenado en la Transmission Control Unit',
    minValue: 0,
    maxValue: 999999,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '0400'),
    name: 'ECM Odometer',
    formula: new Formula('(A<<24|B<<16|C<<8|D)/10'),
    unit: 'km',
    dataBytes: 4,
    pidType: 'formula',
    confidence: 0.9,
    source: 'manual',
    description: 'Kilometraje almacenado en la Engine Control Module',
    minValue: 0,
    maxValue: 999999,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '7A76'),
    name: 'Hybrid Battery State of Charge',
    formula: new Formula('A*0.5'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 0.85,
    source: 'manual',
    minValue: 0,
    maxValue: 100,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '7A53'),
    name: 'Hybrid Battery Voltage',
    formula: new Formula('(A*256+B)*0.01'),
    unit: 'V',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 0.85,
    source: 'manual',
    minValue: 0,
    maxValue: 655.35,
  },
]

/** PIDs propietarios Mode 22 para VAG/Audi (A3 2.0 TDI, Ross-Tech DIDs). */
export const VAG_AUDI_MODE_22_PIDS: PidDefinition[] = [
  {
    id: 0,
    pidCode: new PidCode('22', '1130'),
    name: 'Engine Speed',
    formula: new Formula('(A*256+B)/4'),
    unit: 'rpm',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '115C'),
    name: 'Charge Air Pressure Actual',
    formula: new Formula('A*256+B'),
    unit: 'mbar',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '115E'),
    name: 'Charge Air Pressure Specified',
    formula: new Formula('A*256+B'),
    unit: 'mbar',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F430'),
    name: 'Coolant Temperature',
    formula: new Formula('A'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F432'),
    name: 'Intake Air Temperature',
    formula: new Formula('A'),
    unit: '°C',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F477'),
    name: 'Fuel Rail Pressure Actual',
    formula: new Formula('(A*256+B)*0.01'),
    unit: 'bar',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F47D'),
    name: 'Fuel Rail Pressure Specified',
    formula: new Formula('(A*256+B)*0.01'),
    unit: 'bar',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1035'),
    name: 'EGR Duty Cycle',
    formula: new Formula('A'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1250'),
    name: 'Engine Torque',
    formula: new Formula('A*256+B'),
    unit: 'Nm',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1132'),
    name: 'Injection Quantity',
    formula: new Formula('(A*256+B)*0.01'),
    unit: 'mg/stroke',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1184'),
    name: 'Intake Air Mass',
    formula: new Formula('A*256+B'),
    unit: 'mg/stroke',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1410'),
    name: 'DPF Soot Mass',
    formula: new Formula('(A*256+B)*0.01'),
    unit: 'g',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '140E'),
    name: 'DPF Differential Pressure',
    formula: new Formula('A*256+B'),
    unit: 'mbar',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F449'),
    name: 'Accelerator Pedal Position',
    formula: new Formula('A'),
    unit: '%',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', '1462'),
    name: 'Battery Voltage',
    formula: new Formula('(A*256+B)/100'),
    unit: 'V',
    dataBytes: 2,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
  {
    id: 0,
    pidCode: new PidCode('22', 'F40D'),
    name: 'Vehicle Speed',
    formula: new Formula('A'),
    unit: 'km/h',
    dataBytes: 1,
    pidType: 'formula',
    confidence: 1.0,
    source: 'manual',
    minValue: 0,
    maxValue: 65535,
  },
]

/** Todos los PIDs de catálogo disponibles como seed data.
 * Los PIDs globales (Mode 01 y 09) aplican a cualquier vehículo (vehicle_id = NULL).
 * Los PIDs Mode 22 son específicos de Toyota Auris Hybrid y VAG/Audi.
 */
export const ALL_SEED_PIDS: PidDefinition[] = [
  ...STANDARD_MODE_01_PIDS,
  ...STANDARD_MODE_09_PIDS,
  ...TOYOTA_AURIS_MODE_22_PIDS,
  ...VAG_AUDI_MODE_22_PIDS,
]
