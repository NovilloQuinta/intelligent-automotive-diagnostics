...
/** Base de porcentaje: `value` llega en 0-100, se reescala antes de codificar el byte. */
...
const encodePercent = (value: number): number[] => [
  Math.round((value * PERCENT_SCALE) / PERCENT_BASIS),
]
const encodeFuelTrim = (value: number): number[] => [
  Math.round(((value + FUEL_TRIM_OFFSET) * FUEL_TRIM_SCALE) / PERCENT_BASIS),
]
