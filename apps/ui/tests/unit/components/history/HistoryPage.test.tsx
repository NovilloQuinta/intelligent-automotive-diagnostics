...
describe('HistoryPage', () => {
  ...
  it('should render table rows with date, vehicle, DTC count, and severity', async () => {
    mockUseDiagnosisHistory.mockReturnValue(setHookState({ sessions: SAMPLE_SESSIONS, total: 2 }))
  ...
  it('should show loading skeletons when data is loading', async () => {
    mockUseDiagnosisHistory.mockReturnValue(setHookState({ isLoading: true }))
  ...
  it('should show pagination controls when there are more sessions than the limit', async () => {
    mockUseDiagnosisHistory.mockReturnValue(setHookState({ sessions: SAMPLE_SESSIONS, total: 30 }))
