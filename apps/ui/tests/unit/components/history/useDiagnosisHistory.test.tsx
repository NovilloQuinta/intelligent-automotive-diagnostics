...
describe('useDiagnosisHistory', () => {
  ...
  it('should fetch diagnosis history and return sessions + total', async () => {
    ...
    const { useDiagnosisHistory } =
      await import('../../../../src/components/history/useDiagnosisHistory')

    const { result } = renderHook(() => useDiagnosisHistory({ limit: 25, offset: 0 }), {
  ...
  it('should refetch when filters change', async () => {
    ...
    const { rerender } = renderHook(
      ({ from }: { from?: string }) => useDiagnosisHistory({ limit: 25, offset: 0, from }),
  ...
  it('should expose isError and error when the API fails', async () => {
    vi.mocked(api.getDiagnosisHistory).mockRejectedValue(new Error('Network error'))
