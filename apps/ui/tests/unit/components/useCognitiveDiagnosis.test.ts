...
vi.mock('../../../src/lib/api', () => ({
  ...
  GENERIC_ERROR_MESSAGE: 'Ha ocurrido un problema. Si el problema persiste, contacta con soporte.',
}))
...
describe('useCognitiveDiagnosis', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(api.getCognitiveDiagnosis).mockRejectedValue(new Error('not called'))
  })
