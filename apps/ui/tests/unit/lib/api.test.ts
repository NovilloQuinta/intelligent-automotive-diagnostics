...
describe('api — endpoints', () => {
  ...
  describe('login', () => {
    ...
    it('explains the lockout in Spanish with the remaining minutes on 423', async () => {
      ...
      await expect(api.login({ email: 'a@b.com', password: 'wrong' })).rejects.toThrow(
        /bloqueada temporalmente.*9 minutos/i,
      )
    })
    ...
    it('falls back to a generic wait message when the lockout has no remaining time', async () => {
      ...
      await expect(api.login({ email: 'a@b.com', password: 'wrong' })).rejects.toThrow(
        /bloqueada temporalmente.*más tarde/i,
      )
    })
      ...
      vi.stubGlobal(
        ...
        vi.fn().mockResolvedValueOnce({
          ...
          json: async () => ({
            ...
            details: [{ message: 'email must be valid' }, { message: 'password too short' }],
          }),
        }),
      )
  ...
  describe('getScenarios', () => {
    ...
    it('shares a single refresh across concurrent 401s', async () => {
      ...
      const [first, second] = await Promise.all([api.getScenarios(), api.getScenarios()])
  ...
  describe('runDiagnosis', () => {
    ...
    it('throws the generic message on a 500, never the raw server error', async () => {
      ...
      await expect(api.runDiagnosis('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('getCognitiveDiagnosis', () => {
    ...
    it('posts scenarioId and query and returns the cognitive output', async () => {
      setStoredTokens()
      const mockFetch = vi.fn().mockResolvedValueOnce({ ok: true, json: async () => cognitive })
      vi.stubGlobal('fetch', mockFetch)

      const result = await api.getCognitiveDiagnosis('audi-a3-idle', 'que falla?')
    ...
    it('throws the generic message on a 503, never the raw server error', async () => {
      ...
      await expect(api.getCognitiveDiagnosis('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  ...
  describe('getLiveData', () => {
    ...
    it('propaga los null de cada PID sin convertirlos', async () => {
      ...
      vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce({ ok: true, json: async () => live }))
  ...
  describe('getEcuInfo', () => {
    ...
    it('throws the generic message on a 500, never the raw server error', async () => {
      ...
      await expect(api.getEcuInfo('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('getVehicleInfo', () => {
    ...
    it('throws the curated message when the scenario does not exist', async () => {
      ...
      await expect(api.getVehicleInfo('no-existe')).rejects.toThrow('Scenario not found')
    })
  })
  ...
  describe('getPendingDtc', () => {
    ...
    it('throws the generic message on a 500', async () => {
      ...
      await expect(api.getPendingDtc('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('getPermanentDtc', () => {
    ...
    it('throws the generic message on a 500', async () => {
      ...
      await expect(api.getPermanentDtc('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('clearDtc', () => {
    ...
    it('throws the generic message on a 500', async () => {
      ...
      await expect(api.clearDtc('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('getVehicleStatus', () => {
    ...
    it('throws the generic message on a 500', async () => {
      ...
      await expect(api.getVehicleStatus('audi-a3-idle')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('forgotPassword', () => {
    ...
    it('throws the generic message on a 500', async () => {
      ...
      await expect(api.forgotPassword('a@b.com')).rejects.toThrow(GENERIC_ERROR_MESSAGE)
    })
  })
  ...
  describe('resetPassword', () => {
    it('posts the token and new password and resolves on 200', async () => {
      ...
      await expect(api.resetPassword('raw-token', 'Password123!')).resolves.toBeUndefined()
    ...
    it('throws the curated error message on a 400 (invalid/expired token)', async () => {
      ...
      await expect(api.resetPassword('bad-token', 'Password123!')).rejects.toThrow(
  ...
  describe('updateProfile', () => {
    ...
    it('throws the curated error message on a 409 (username taken)', async () => {
      ...
      await expect(api.updateProfile({ username: 'taken' })).rejects.toThrow(
  ...
  describe('changePassword', () => {
    it('sends currentPassword/newPassword with the Bearer token', async () => {
      ...
      await expect(api.changePassword('OldPass123!', 'NewPass456!')).resolves.toBeUndefined()
    ...
    it('throws the curated error message on a 401 (wrong current password)', async () => {
      ...
      await expect(api.changePassword('wrong', 'NewPass456!')).rejects.toThrow(
