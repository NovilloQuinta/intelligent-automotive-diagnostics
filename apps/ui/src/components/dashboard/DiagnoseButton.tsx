...
type Props = {
  loading: boolean; disabled: boolean; onClick: () => void; rawSummary: string | null
}

/** CTA button with loading state and raw OBD frame readout. */
export function DiagnoseButton({ loading, disabled, onClick, rawSummary }: Props) {
  return (
    <div className="mt-5 flex flex-wrap items-center gap-4">
      ...
      >
        {loading ? (<><Loader2 className="h-4 w-4 animate-spin" />Diagnosticando…</>)
          : (<><ScanLine className="h-4 w-4" />Iniciar diagnóstico</>)}
      </button>
      <div className="mono flex-1 truncate rounded-md border border-white/5 bg-black/40 px-3 py-2 text-[11px] text-muted-foreground">
        <span className="text-primary">RAW ›</span> {rawSummary ?? 'Aguardando trama OBD-II…'}
      </div>
    </div>
  )
}
