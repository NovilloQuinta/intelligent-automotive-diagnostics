...
type TelemetryValues = {
  rpm: number | null; coolant: number | null; speed: number | null; intake: number | null
}
...
function LiveBadge({ streamOk, loading }: { streamOk: boolean; loading: boolean }) {
  ...
  return (
    <span className="ml-2 inline-flex items-center gap-1.5 rounded-md border border-[#00d4aa]/30 bg-[#00d4aa]/10 px-2 py-0.5">
      ...
      <span className="mono text-[10px] font-bold uppercase tracking-widest" style={{ color: COLORS.accent }}>Live</span>
    </span>
  )
}

/** Left panel: live telemetry gauges + diagnose button with streaming status. */
export function TelemetrySection({ values, rawSummary, loading, streamOk, canDiagnose, telemetryStatus, onDiagnose }: Props) {
  const { rpm, coolant, speed, intake } = values
  return (
    <section className={`panel relative flex flex-col overflow-hidden p-4 md:p-5 ${loading ? 'scanning' : ''}`}>
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="mono text-[10px] uppercase tracking-widest text-muted-foreground">{telemetryStatus}</div>
      </div>
      <div className={`relative grid flex-1 grid-cols-1 gap-4 sm:grid-cols-2 ${loading ? 'scan-sweep' : ''}`}>
      ...
      <DiagnoseButton loading={loading} disabled={!canDiagnose} onClick={onDiagnose} rawSummary={rawSummary} />
    </section>
  )
}
