  ...
  return (
    <div className="panel flex min-h-0 flex-col p-4">
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        {text && meta && !loading && (
          <DiagnosisContent text={text} meta={meta} Icon={Icon} />
        )}
      </div>
    </div>
  );
...
function DiagnosisHeader({
  ...
  meta: ReturnType<typeof severityMeta> | null;
}) {
  return (
...
function DiagnosisContent({
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-2 whitespace-pre-line text-foreground/90">
        {text}
      </div>
    </div>
  );
