...
- **Rama base**: `develop`, con `add-password-recovery-and-profile` reintegrado tras un corte de entorno que borró el worktree a medio camino (rama `claude/client-profile-password-recovery-2fwi91` reconstruida commit a commit y remergeada con el `develop` mas reciente).
...
- **13 OpenSpec changes archivados** (2026-08-09): add-admin-management-panel, add-obd-standard-modes, fix-cognitive-diagnosis-ux, fix-emulator-coherent-data, fix-vehicle-identity-and-live-data, add-knowledge-mcp-tools, add-vehicle-autodetect-flow, rebase-ui-autel-sidebar-dtc-badge, add-usb-serial-connection-type, add-monitor-reset-on-clear-dtc (discarded), add-user-profiles (discarded), add-diagnosis-history, add-diagnosis-sqlite-persistence, add-password-recovery-and-profile.
