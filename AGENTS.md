...
- **Ultimo paso**: `fix/live-data-pid-selector` mergeado a `develop` (commit `5fbdc91`). Corrige la feature archivada `add-live-data-pid-selector`: (1) `Elm327TcpRepository.readPids` pasó de un único comando multi-PID (`01 0C 05 0D 0F`, que el ELM327-emulator no soporta → `NO DATA` → gauges en 0) a lectura secuencial con degradación por PID; (2) nuevo `GET /api/available-pids` (catálogo SAE J1979 de 16 Mode 01) + `PidsTable` renderiza los 16 como filas seleccionables (4 por defecto, máx 8). e2e Playwright corregidos a la realidad docker (`Audi A3 2.0 TDI`, 3 DTCs con freeze frame, wizard) — 12/12 pass. GGA bloqueó por deuda preexistente (DRY en DiagnosisController + bug contrato historial), documentada en `docs/gga-pending-errors.md`.
