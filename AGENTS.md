...
- **Ultimo paso**: Deploy CI/CD a `diag.jcodinglabs.com` completado. Pipeline `deploy.yml` (push a `main`): builda 3 imagenes a GHCR (`ghcr.io/novilloquinta/intelligent-automotive-diagnostics/{api,ui,elm327}`) y despliega por SSH al VPS OVH (145.239.77.72) via `docker compose -f docker-compose.prod.yml` en `/opt/intelligent-automotive-diagnostics`. Caddy proxy: `/api`→:4000, UI→nginx:8080. Fixes necesarios: `tsc-alias` (alias `@/*` no se resolvia con `node dist/main.js`), copiar `apps/core-api/drizzle` al contenedor, healthcheck con `node fetch` (wget no existe en node:22-slim), base `node:22-slim` (prebuilds nativos), nombres GHCR en minusculas, secrets `VPS_HOST/VPS_USER/VPS_SSH_KEY`, `gh auth refresh -s workflow` (pushear .github/workflows). Verificado en VPS: contenedores healthy + register/login/scenarios OK (3 coches emulados, `OBD_MODE=docker`). Web publica y funcionando: registro A `diag.jcodinglabs.com` → `145.239.77.72` hecho en IONOS (el subdominio `diag` era un hosting IONOS con A+AAAA+MX+SPF; se borro y se puso registro A suelto), cert Let's Encrypt emitido por Caddy, HTTPS OK. Healthchecks docker corregidos (elm327 sin probe `service_started`; ui/api `127.0.0.1` no `localhost`).
