...
Capacidad de lectura de datos en vivo multi-PID: `readPids` envía un único comando ELM327 para leer varios PIDs Mode 01, el endpoint `GET /api/live-data` acepta un query param opcional `pids` para seleccionar dinámicamente los PIDs devueltos, y la UI permite seleccionar mediante checkboxes qué PIDs se muestran en los gauges de telemetría en vivo.
