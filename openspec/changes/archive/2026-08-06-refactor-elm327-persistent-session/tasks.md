...
- [x] 0.1 Crear rama `feat/refactor-elm327-persistent-session` desde `main`
- [x] 0.2 Verificar baseline: `pnpm lint && pnpm test && pnpm build` verdes en `main`
- [x] 0.3 Cargar skills: `openspec-apply-change`, `tdd-workflow` (para el writer)
...
- [x] 1.1.1 Re-escribir `apps/core-api/tests/unit/infrastructure/elm327/tcpTransport.test.ts`:
...
- [x] 1.2.1 Reescribir `createElm327TcpClient` en `tcpTransport.ts`:
...
### 1.3 REFACTOR
- [x] 1.3.1 Verificar que los tests de `tcpTransport.test.ts` pasan en verde
- [x] 1.3.2 `pnpm lint` limpio
- [x] 1.3.3 Revisar TSDoc — documentar el nuevo comportamiento persistente en el JSDoc de `createElm327TcpClient`
...
- [x] 2.1.1 Actualizar `apps/core-api/tests/unit/infrastructure/elm327/elm327Adapter.test.ts`:
...
- [x] 2.2.1 Modificar `Elm327TcpRepository` en `elm327Adapter.ts`:
...
### 2.3 REFACTOR
- [x] 2.3.1 Verificar que los tests del adapter pasan en verde
- [x] 2.3.2 `pnpm lint` limpio
- [x] 2.3.3 Actualizar TSDoc del adapter para documentar `connect()` eager y `close()`
...
- [x] 3.1.1 Actualizar `apps/core-api/tests/unit/usecases/ProcessVehicleDiagnosisUseCase.test.ts`:
...
- [x] 3.2.1 Reescribir `execute()` en `ProcessVehicleDiagnosisUseCase`:
...
### 3.3 REFACTOR
- [x] 3.3.1 Verificar que los tests del use case pasan en verde
- [x] 3.3.2 `pnpm lint` limpio
- [x] 3.3.3 Si `withTimeout` no se usa en ningún otro módulo, eliminarla. Si se usa, dejarla.
...
- [x] 4.1 `pnpm test` → todos los tests pasan (tcpTransport, elm327Adapter, ProcessVehicleDiagnosisUseCase + suite completa)
- [x] 4.2 `pnpm lint && pnpm format` → sin errores
- [x] 4.3 `pnpm build` → compila sin errores
- [ ] 4.4 Verificación manual opcional: `printf '01 0C\r\n' | nc -w 2 localhost 35000` (emulador Docker) si está corriendo
- [x] 4.5 Actualizar `AGENTS.md` (SESION ACTUAL: cambio `refactor-elm327-persistent-session` completado)
