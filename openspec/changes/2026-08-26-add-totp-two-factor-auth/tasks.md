...
- [x] 0.1 Baseline en verde (`pnpm test`) y rama comprobada con `git branch --show-current`
- [x] 0.2 Instalar `otplib` y `qrcode` (+ `@types/qrcode`) en `apps/core-api`
- [x] 0.3 Contexto: `LoginUserUseCase`, `authService.ts`, `admin.middleware.ts`,
...
- [x] 7.1 `pnpm verify` en verde
- [x] 7.2 `pnpm test:coverage` sin bajar ningun umbral (los ficheros nuevos no estan excluidos)
- [x] 7.3 Prueba manual con una app TOTP real (Aegis / Google Authenticator)
- [x] 7.4 `docs/security.md`: cerrar el residual 2; anotar el cifrado del secreto en el 3;
      filas A02/A07 y API2
- [x] 7.5 `docs/deuda-conocida.md`: **re-medir**, no estimar
- [x] 7.6 `docs/estado-actual.md`: max 15 lineas, y avisar de la ruptura para administradores
- [x] 7.7 Avisar del bullet de la slide 17 (`'La ausencia de MFA'`), que vive fuera de `develop`
