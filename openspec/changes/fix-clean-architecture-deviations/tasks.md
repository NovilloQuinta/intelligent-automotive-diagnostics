...
- [ ] 0.1 Crear rama `fix/clean-architecture-deviations` desde `main`
- [ ] 0.2 Verificar baseline: `pnpm lint && pnpm test && pnpm build` verdes en `main`

## 1. Inyectar `LoggerPort` en `ExecuteLlmToolCalling` (TDD)

- [ ] 1.1 RED: Actualizar `tests/unit/usecases/llm/executeLlmToolCalling.test.ts`:
...
- [ ] 2.2 GREEN: Crear `src/infrastructure/services/diagnosisService.ts`:
...
- [ ] 2.3 Suite del servicio verde + `pnpm lint`

## 3. Refactor `DiagnosisController` para recibir `DiagnosisService` (REFACTOR)

- [ ] 3.1 Actualizar `DiagnosisController`:
...
- [ ] 3.5 Suite completa verde + `pnpm lint`
