...
El sistema SHALL descubrir las ECUs del vehículo enviando un broadcast por functional addressing (`AT SH 7DF`) y recogiendo las direcciones de respuesta de los headers CAN. Cada ECU que responda SHALL mapearse a una `EcuInfo` con `responseAddr` (header), `requestAddr` (header − 8), y `type`/`name` resueltos desde el catálogo ISO 15765-4 — que solo estandariza `7E0/7E8 = ECM`; el resto SHALL resolverse como `UNKNOWN`.

#### Scenario: Broadcast multi-ECU descubre varias ECUs
- **GIVEN** un `Elm327Transport` conectado que, tras `AT H1` + `AT SH 7DF`, responde a `01 00` con `7E8 06 41 00 BE 3F A8 13` y `7E9 06 41 00 ...`
- **WHEN** se invoca `getEcuInfo()` en `Elm327TcpRepository`
- **THEN** devuelve 2 `EcuInfo`: la `7E8` con `type = 'ECM'` (7E0→7E8) y la `7E9` con `type = 'UNKNOWN'` (7E1→7E9)
- **AND** `requestAddr` se deriva aritméticamente (`response − 8`)

#### Scenario: Secuencia AT emitida en orden
- **GIVEN** un mock de `Elm327Transport` que registra los comandos recibidos
- **WHEN** se ejecuta el auto-scan
- **THEN** los comandos se emiten en el orden `AT E0`, `AT L0`, `AT H1`, `AT SP 6`, `AT SH 7DF`, y después el broadcast `01 00`
...
#### Scenario: Headers fuera del rango OBD se ignoran
- **GIVEN** una respuesta con headers `7E8`, `7EF` y `18DAF110` (29-bit)
- **WHEN** se parsea la respuesta
- **THEN** solo se devuelven ECUs para los headers 11-bit en rango `7E8–7EF`; `18DAF110` se descarta
...
El sistema SHALL caer a Mode 09 PID 0A (ECU name) con addressing físico (`AT SH 7E0`) cuando el broadcast por functional addressing no produzca ninguna respuesta, devolviendo una única `EcuInfo` ECM (`7E0`/`7E8`). Si tampoco responde, SHALL devolver `[]`.

#### Scenario: Broadcast vacío y 09 0A responde
- **GIVEN** un transporte que a `01 00` (functional) devuelve vacío, y a `AT SH 7E0` + `09 0A` responde `49 0A 01 ...`
- **WHEN** se invoca `getEcuInfo()`
- **THEN** devuelve una `EcuInfo` ECM (`7E0`/`7E8`)
